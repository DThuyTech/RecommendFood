...
//    List<CategoryFood> mListcategory = new ArrayList<CategoryFood>();
    ...
    public List<Food> logicfunc(List<CategoryFood> mListcategory,List<String> listAnswer,List<Food> mListFoRaw)
    {
...
//            txtTest.setText(tesst);
        ...
        mlistend = randomCaFood(nodeEnd);
        List<Food> foods = new ArrayList<Food>();




        return foods = getFood(mlistend,mListFoRaw);
...
//                txtTest.setText(finaltest);
    ...
    public List<CategoryFood> randomCaFood(Node node) {
    ...
    public List<Food> getFood(List<CategoryFood> categoryFoods,List<Food> mListRaw) {
        List<Food> mlisFood = new ArrayList<Food>();
        Food food;
        List<Food> Listfoodraw = new ArrayList<Food>();
        food = new Food();
        int Id_Cate = categoryFoods.get(0).getId();

       for(Food food1 : mListRaw){
           if(food1.getCategory_id()==Id_Cate){
               Listfoodraw.add(food1);
           }
       }
       int random =  (int) Math.floor(Math.random() * (Listfoodraw.size()-1 - 0 + 1) + 0);
       mlisFood.add(Listfoodraw.get(random));

        Id_Cate = categoryFoods.get(1).getId();
    Listfoodraw = new ArrayList<Food>();
        for(Food food1 : mListRaw){
            if(food1.getCategory_id()==Id_Cate){
                Listfoodraw.add(food1);
            }
        }
         random =  (int) Math.floor(Math.random() * (Listfoodraw.size()-1 - 0 + 1) + 0);
        mlisFood.add(Listfoodraw.get(random));
